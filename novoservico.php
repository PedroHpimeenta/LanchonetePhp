<?php
    ...
    @$nome = $_POST['nome'];
    @$capa = $imagem;
    @$depoimento = $_POST['descricao'];
    @$preco = $_POST['preco'];
   

    if($nome == null ||   $capa == null || $depoimento == null || $preco == null) {
        echo msg_aviso("Existem campos em branco, favor preencher todos os campos");
        require_once 'novoservico-form.php';
    }else{
        $sql_prod = $banco->query("insert into secao4 (nome_servico,descricao,preco,img) values ('$nome','$depoimento','$preco','$capa')");
        echo msg_sucesso("novo serviço cadastrado com sucesso");
